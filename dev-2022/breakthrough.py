...
class Breakthrough():
	...
	def __SetupGame(self):
		...
		if Choice == "L":
			...
			if not self.__LoadGame(f"dev-2022/{FileName}.txt"):
				self.__GameOver = True
		else:
...
class CardCollection():
	...
	def DisplayStats(self):
		...
		Msg = f"There is a {PercentageKeys:.2f}% chance that the next card will be a key, a {PercentageFiles:.2f}% chance that it will be a file and a {PercentagePics:.2f}% chance that it will be a pick."
