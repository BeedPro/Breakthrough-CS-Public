...
class Breakthrough():
    ...
    def __SetupGame(self):
        ...
        if Choice == "L":
            if not self.__LoadGame("dev-2022/game1.txt"):
    ...
    def __AddWildcardCardsToDeck(self):
        """
        adds wildcard cards to the deck
        """
        NewCard = ToolCard("P", "w")
        self.__Deck.AddCard(NewCard)
        NewCard = ToolCard("F", "w")
        self.__Deck.AddCard(NewCard)
        NewCard = ToolCard("K", "w")
        self.__Deck.AddCard(NewCard)
